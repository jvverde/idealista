...
/*    function extractPropertyInfo(item) {
        ...
        return {
            id: idMatch[1],
            url,
            price: parseInt(item.querySelector('.price-row .item-price')?.textContent?.replace(/\D+/g, '') || 0,
            typology: item.querySelector('.item-detail-char .item-detail')?.textContent?.match(/(T\d+|Quarto|Estúdio)/i)?.[0] || 'N/A',
            ...
            hasGarage: item.querySelector('.item-parking, [title*="garagem"]') !== null,
            isActive: true
        };
    }*/
    function extractPropertyInfo(item) {
                    ...
                    <tbody>
                        ${displayData.map(prop => `
                            <tr>
                                ...
                                <td class="${prop.isActive ? 'status-active' : 'status-inactive'}">
