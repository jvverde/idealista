    ...
    const setupTableSorting = () => {
        ...
        headers.forEach((header, index) => {
            ...
            header.addEventListener('click', () => {
                ...
                header.innerHTML += `<span class="sort-arrow">${currentSort.direction === 1 ? '↑' : '↓'}</span>`;
                ...
                rows.sort((a, b) => {
                    ...
                    if (index === 0) return aVal.localeCompare(bVal) * currentSort.direction;
                    if (index === 1) return (parsePrice(aVal) - parsePrice(bVal)) * currentSort.direction;
                    if (index === 3) return (parseArea(aVal) - parseArea(bVal)) * currentSort.direction;
                    if ([4,5].includes(index)) return (new Date(aVal) - new Date(bVal)) * currentSort.direction;
                    return 0;
                });
