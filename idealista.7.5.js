...
(function() {
    ...
    function isIdealistaListingPage() {
        ...
        const forbiddenPaths = [
            ...
            /\/contato\//,
            /\/mapa$/,
            /pagina-\d+/  // Add this to exclude paginated pages
        ];
    ...
    function getStorageKey(context) {
        if (context.isAreaSearch) {
            return `${STORAGE_PREFIX}area_${context.transactionType}_${context.propertyType}_${context.ordem}`;
        }
        return `${STORAGE_PREFIX}${context.transactionType}_${context.propertyType}_${context.location}_${context.subLocation}_${context.ordem}`.replace(/_+$/, '');
    }
 ...
 #idealistaPanel {
    ...
    function setupTableSorting() {
        ...
        headers.forEach((header, index) => {
            ...
            if (['Tipo', 'Preço', 'Área', '1ª Detecção', 'Últ. Atualização'].includes(text)) {
                header.classList.add('sortable-header');
                header.addEventListener('click', () => {
                    // Remove previous sort indicators
                    headers.forEach(h => {
                        h.innerHTML = h.textContent; // Clear any existing arrows
                    });
                    ...
                    header.innerHTML = `${text}<span class="sort-arrow">${arrow}</span>`;

                    // Sort the table
                    console.log('index',index, currentSort.direction)
    ...
    function sortTable(table, columnIndex, direction) {
        ...
        rows.sort((a, b) => {
            ...
            switch (columnIndex) {
                ...
                case 1: // Preço
                    aValue = parsePrice(aCell.textContent);
                    bValue = parsePrice(bCell.textContent);
                    return (aValue - bValue) * direction;

                case 3: // Área
                    aValue = parseArea(aCell.textContent);
                    bValue = parseArea(bCell.textContent);
                    return (aValue - bValue) * direction;

                case 4: // 1ª Detecção
                case 5: // Últ. Atualização
    ...
    function parseArea(areaText) {
        ...
        console.log('MATCH parsearea', match);
        return match ? parseInt(match[1]) : 0;
    }

    // Create de UI interface
    async function createUI() {
        ...
        try {
            ...
            panel.querySelector('#idealistaClearContext').addEventListener('click', async () => {
                ...
                if (confirm(`⚠️ ATENÇÃO: Isso apagará todos os dados armazenados para:\n\n"${translateContext(currentContext)}"\n\nContinuar?`)) {
                    try {
                        await GM.deleteValue(currentKey);
                        alert(`Dados removidos para:\n\n"${translateContext(currentContext)}"`);
                        panel.remove();
                    } catch (e) {
    ...
    function translateContext(context) {
        ...
        return `${transactions[context.transactionType]} ${properties[context.propertyType]}${locationText} | ${sortingTypes[context.ordem] || context.ordem}`;
    }
    ...
    // Update the refreshUI function to clear the timeout
