...
(() => {
    ...
    const updatePropertyStatus = (id, isActive) => {
        Object.keys(allDataCache).forEach(key => {
            if (allDataCache[key][id]) {
                allDataCache[key][id].lastSeen = getCurrentISODate();
    ...
    const findOldestPropertyRecord = async (propertyId) => {
        const allKeys = Object.keys(allDataCache);
        let oldestRecord = null;

        for (const key of allKeys) {
            if (allDataCache[key][propertyId]) {
                const record = allDataCache[key][propertyId];
                if (!oldestRecord || new Date(record.firstSeen) < new Date(oldestRecord.firstSeen)) {
                    oldestRecord = record;
                }
            }
        }
            ...
            if (!ctx.isAreaSearch) {
                await Promise.all(Object.keys(data).map(async id => {
                    if (!currentIds.includes(id)) {
                        ...
                        data[id].lastChecked = getCurrentISODate();
                    }
                }));
            }
